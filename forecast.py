...
#No longer used
#HFA = 65.0     # Home field advantage was worth 65 Elo points

#Change HFA to account for different teams strength of HFA
#http://www.espn.com/nfl/story/_/id/20371914/home-field-advantage-nfl-2017-toughest-easiest-teams-play-road-more

#Also try and account for injuries somehow from Sports Reference
...
class Forecast:
    ...
    def forecast(games):
        ...
        teams = {}
        for row in [item for item in csv.DictReader(open("C:/Users/User/Documents/Coding Projects/NFL Predictions Game/nfl-elo-game-master/nfl-elo-game-master/data/initial_elos.csv"))]:
            teams[row['team']] = {
                'name': row['team'],
                'season': None,
                'elo': float(row['elo']),
#Introduce Different HFAs
                'hfa': float(row['hfa']),
#Include Injuries
                'inj':float(row['inj'])
            }
        ...
        for game in games:
            ...
            elo_diff = team1['elo'] - team2['elo'] + (0 if game['neutral'] == 1 else team1['hfa'])#get HFAs matching up here
            
            #Include Injury info in Elo difference
            elo_diff_inj = elo_diff + team2['inj'] - team1['inj']

            # This is the most important piece, where we set my_prob1 to our forecasted probability
            if game['elo_prob1'] != None:
                game['my_prob1'] = 1.0 / (math.pow(10.0, (-elo_diff_inj/400.0)) + 1.0)
